...
> Note: React supports any browser event you will ever need as documented by [this list of supported events](https://react.dev/reference/react-dom/components/common)
...
- The names for event props are camelCased (`onClick`). In HTML, the attribute would be `onclick`. Here's the [list of events](https://react.dev/reference/react-dom/components/common) supported by React.
...
This scenario, is what React refers to as a [controlled component/input](https://react.dev/learn/sharing-state-between-components#controlled-and-uncontrolled-components).
...
Here is the completed [Fundamentals of React Chart](https://git.generalassemb.ly/SEIR-2-21-23/course-materials/blob/main/resources/react-fundamentals.md) that is excellent to review from time-to-time, make flash cards from, or read before bedtime 😃
...
- [React Docs - Event Object](https://react.dev/reference/react-dom/components/common#react-event-object)
