...
## Setup
  ...
  - In the folder that you keep your labs, run `npm create vite@latest`.
  - Give it the title `react-students-lab`, choose the React framework, and Javascript variant.
  - `cd` into the `react-students-lab` folder, run `npm install`, open it up with a `code .`, and `npm run dev` in your terminal.
